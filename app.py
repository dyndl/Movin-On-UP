...
for LA_zipcode in LA_zipcodes:
    ...
    if LA_zipcode not in LA_other_zipcodes:
        url = path + "ZILLOW2/Z"+LA_zipcode+"_MSPAHforecast.csv"
        #print(url)
        try:
            ...
            print("***File ",url, " doesn't exist!***")

#print(count)
column_labels = ['Rankings', 'Zipcode', 'Trend','Avg. ROI','ROI Rank' ,'Wealth Rank','Latitude','Longitude']
...
ranked_data = pd.DataFrame(columns = column_labels) #[np.null(150,len(column_labels))],
